...
layui.config({
    ...
    table.render({
        id:'wanguiList',
        elem:'#wanguiList',
        url:ctx+"/wangui/getWanguiList", //数据接口
        cellMinWidth:80,
        //toolbar:true,
        //title:'打扫记录表',
        //totalRow:true,
        ...
        cols:[[ //表头
            ...
            {field:'wanguiTime', title: '晚归时间' ,align: 'center',templet : '<div>{{ formatTime(d.wanguiTime,0,"yyyy-MM-dd hh:mm:ss")}}</div>' },

            {field:'dormitoryNumber',title:'宿舍名称',align:'center',templet: '<div>{{d.dormitory.dormitoryNumber}}</div>'},
            {field:'username',title:'晚归人姓名',align:'center',templet: '<div>{{d.student.username}}</div>'},
            {title:'操作',toolbar:'#barEdit',align:'center'}
        ]],
        page:true
    });
    //监听工具条
    table.on('tool(wanguiList)',function (obj) {
        var data=obj.data;
        if(obj.event=='del')
        {
            ...
            layer.confirm('真的删除吗？',function (index) {
                $.ajax({
                    url:ctx+"/wangui/delWangui/"+data.id,
                    type:"get",
                    success:function (d) {
                        if(d.code==0)
                        {
                            table.reload('wanguiList',{});//重新发送请求 表格重载
                        }
                        else
                        {
    ...
    //添加角色
    $(".wanguiAdd_btn").click(function () {
        var index=layer.open({
            title:"添加晚归记录",
            type:2,
            content:ctx+"/wangui/addWangui",
            area:['380px','550px']
        });
